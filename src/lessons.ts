...
export type TipCard = {
  ...
  how: string;
  /** Grupo temático para agrupar tarjetas con encabezado (primer tip del grupo lo define). */
  group?: string;
};
  ...
  },
  {
    ...
    tips: [
      ...
      },
      {
        ...
        emoji: "🩺",
        icon: "health",
        title: "Diagnostica tu instalación",
        what: "Revisa en segundos si tu Claude Code está sano: detecta problemas de configuración, servidores MCP que no cargan y skills rotos, y muestra el estado de cada componente con iconos claros.",
        how: "Escribe `/doctor` en cualquier momento. Si detecta algo roto, pulsa `f` para que Claude intente corregirlo automáticamente.",
      },
      // ── Contexto y memoria ────────────────────────────────────────────────────
      {
        group: "Contexto y memoria",
        emoji: "🗂️",
        icon: "bookmark",
      ...
      {
        group: "Permisos y configuración",
        emoji: "🧠",
        icon: "lightbulb",
      ...
      },
      {
        ...
        emoji: "🎚️",
        icon: "sliders",
        title: "Ajusta tus preferencias",
        what: "Abre los ajustes de tu sesión: tema, modelo, nivel de razonamiento, modo de editor (vim / normal), memoria automática e idioma. Los cambios se guardan en `~/.claude/settings.json` (usuario) o `.claude/settings.json` (proyecto).",
        how: "Escribe `/config` para abrir la interfaz de ajustes. También puedes pasar un valor directo: `/config theme=dark`.",
      },
      // ── Personalización ───────────────────────────────────────────────────────
      {
      ...
      },
      {
        ...
        emoji: "⚡",
        icon: "bolt",
        title: "Modo liviano para scripts",
        what: "Arranca Claude Code sin cargar skills, plugins, MCP, hooks ni auto-memoria —solo Bash, lectura y edición de archivos—. Reduce el tiempo de arranque e ideal para invocaciones automatizadas en pipelines o CI/CD.",
        how: "Usa `claude --bare` al lanzar desde línea de comandos, o combínalo con `-p` para scripts: `claude --bare -p \"resume este log\"`.",
      },
      // ── Conexión con el mundo ─────────────────────────────────────────────────
      {
        group: "Conexión con el mundo",
        emoji: "🔌",
        icon: "plug",
      ...
      },
      {
        ...
        emoji: "📐",
        icon: "code",
        title: "Salida estructurada garantizada",
        what: "En modo no interactivo (`-p`), fuerza a Claude a devolver un JSON que cumple exactamente el schema que defines. Si la respuesta no valida, el modelo reintenta solo. Perfecto para automatizaciones donde la salida va a otra herramienta.",
        how: "Combínalo con `-p`: `claude -p --json-schema '{\"type\":\"object\",...}' \"analiza el log\"`. El resultado es solo el JSON final, sin texto extra.",
      },
      {
        group: "Conexión con el mundo",
        emoji: "🐙",
        icon: "github",
        title: "Conecta Claude a tus PRs",
        what: "Instala la GitHub App de Claude en tu repo para que responda a `@claude` en comentarios de pull requests e issues —directamente en GitHub Actions, sin abrir Claude Code—. Requiere admin del repositorio y una `ANTHROPIC_API_KEY` como secret.",
        how: "Escribe `/install-github-app` y sigue el flujo interactivo: elige repo, instala la app y configura el secret. Diferente al MCP de GitHub (que es para sesiones interactivas).",
      },
      // ── Agentes y automatización ──────────────────────────────────────────────
      {
        group: "Agentes y automatización",
        emoji: "🪂",
        icon: "fanout",
      ...
      },
      {
        ...
        emoji: "🔁",
        icon: "clock",
        title: "Repite tareas en intervalos",
        what: "Ejecuta un prompt de forma recurrente mientras Claude Code está abierto —ideal para hacer polling de un build, un deploy o el estado de una pipeline sin vigilar la terminal—. El intervalo lo fijas tú, o lo elige Claude. Se pausa con Esc.",
        how: "`/loop 5m revisa si el build terminó` (intervalo fijo), o `/loop revisa el estado` (Claude elige el ritmo). Disponible como skill bundled incluido con Claude Code.",
      },
      {
        group: "Agentes y automatización",
        emoji: "📅",
        icon: "calendar",
        title: "Rutinas automáticas en cloud",
        what: "Crea rutinas que corren en infraestructura de Anthropic aunque apagues tu máquina: chequeos nocturnos, reportes periódicos o mantenimiento que no requiere tu presencia. Mínimo una hora entre ejecuciones.",
        how: "Escribe `/schedule` y sigue el asistente interactivo. La rutina clona tu repo en cloud; usa `/routines` para listarlas o cancelarlas.",
      },
    ],
    takeaway:
